...
const GenericController = {
    ...
    create: async (req, res) => {
        try {
            ...
            // 🌟 הפרדת מפתח האבטחה החוצה כדי שלא יישלח כעמודה ל-MySQL
            const { current_user_id, ...dataToInsert } = req.body;

            // קריאה למודל הגנרי ליצירת השורה החדשה בדאטאבייס עם הנתונים הנקיים
            const newItem = await GenericModel.create(table, dataToInsert);
    ...
    // 4. מחיקת שורה קיימת מהדאטאבייס (בקשת DELETE גנרית)
    delete: async (req, res) => {
        try {
            ...
            // 🌟 הצלבת נתונים חכמה לפי סוג הטבלה: 🌟
            // אם אנחנו בטבלת המשתמשים עצמה, המפתח להצלבה הוא 'id'. בכל טבלה אחרת המפתח הוא 'user_id'.
            const ownerIdInDb = table === 'users' ? itemInDb.id : itemInDb.user_id;

            // אבטחה שלב ג': בודקים האם בעל הרשומה בדאטאבייס שווה ל-ID של מי שמנסה למחוק
            if (String(ownerIdInDb) !== String(current_user_id)) {
    ...
    // 5. עדכון שורה קיימת בדאטאבייס (בקשת PATCH גנרית)
    update: async (req, res) => {
        try {
            const table = req.params.table; // שם הטבלה (למשל: users או posts)
